...
class SocialIconController extends Controller
{
    ...
    {
        return view('backend.social_icon.index');
    }

    public function getdata(Request $request)
    {
        if ($request->ajax()) {
            $data = SocialIcon::orderBy('created_at', 'desc')->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $editUrl = route('social.icon.edit', $row->id);
                    $deleteUrl = route('social.icon.distroy', $row->id);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json(['success' => true, 'message' => 'Social Icon Created Successfully!',]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        ...
        return view('backend.social_icon.edit', compact('data'));
    }

    public function update(Request $request, $id)
    {
        $find = SocialIcon::find($id);
        $request->validate([
            'name' => 'nullable|string|max:255',
            'url' => 'required|string',
            'image' => 'required|file|image|max:2048',
        ...
        $find->update($data);
        return response()->json(['success' => true, 'message' => 'Social Icon Updated Successfully!',]);
    }
    ...
    public function distroy($id)
    {
        ...
        $find->delete();
        Alert::success('Success', 'Social Icon deleted Successful!');
