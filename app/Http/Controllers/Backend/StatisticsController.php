...
class StatisticsController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Statistic::orderBy('created_at', 'desc')->get();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $deleteUrl = route('statistics.distroy', $row->id);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json(['success' => true, 'message' => 'Statistics Created Successfully!',]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        ...
        return view('backend.statistics.edit', compact('data'));
    }

    public function update(Request $request, $id)
    {
        $find = Statistic::find($id);
        $request->validate([
            'title' => 'required|string|max:255',
            'value' => 'required|string|max:255',
        ...
        if ($request->hasFile('image')) {
            if ($find->icon !== null) {
                $imagePath = public_path('image/' . $find->icon);
        ...
        $find->update($data);
        return response()->json(['success' => true, 'message' => 'Statistics Updated Successfully!',]);
    }
    ...
    public function distroy($id)
    {
        ...
        $find->delete();
        Alert::success('Success', 'Statistics deleted Successful!');
